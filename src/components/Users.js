            ...
            snapshot.forEach(itemUser=>{
                if(userid !== ''){
                    ...
                    if(userid !== itemUser.key){
                      userArr.push({"btnText": "Add Friend", dp: itemUser.val().dp, email: itemUser.val().email, userid: itemUser.val().id, username: itemUser.val().username}); 
                        onValue(reqRef, (snapshot) => {
                            snapshot.forEach(itemReq=>{
                                //checking if request sent to user
                                if(userid === itemReq.val().sender_id && itemUser.key === itemReq.val().receiver_id){
                                    userArr.pop();
                                    userArr.push({"btnText": "Request Sent", dp: itemUser.val().dp, email: itemUser.val().email, userid: itemUser.val().id, username: itemUser.val().username}); 
                                }
                                //checking if request got from user
                                else if(userid === itemReq.val().receiver_id && itemUser.key === itemReq.val().sender_id){
                                    userArr.pop();
                                    userArr.push({"btnText": "Pending Request", dp: itemUser.val().dp, email: itemUser.val().email, userid: itemUser.val().id, username: itemUser.val().username});
                                } 
                            })
                            setChange3(!change3);
                        });
                        ...
                        onValue(friendRef, (snapshot) => {
                          snapshot.forEach(itemFriend=>{ 
                            if((itemUser.key === itemFriend.val().sender_id && userid === itemFriend.val().accepter_id)
                              ...
                              //taking users including friends
                              userArr.pop();
                              userArr.push({"btnText": "Friends", dp: itemUser.val().dp, email: itemUser.val().email, userid: itemUser.val().id, username: itemUser.val().username});
                            }  
                        })
                        setChange2(!change2);
                      });
                  ...
                  <Col lg={2} md={2} sm={4}>
                    <Card key={index} style={{ width: '100%' }}>
                        ...
                        <Card.Body>
                            ...
                            :
                            user.btnText == "Friends"
                            ?
                            <Button variant = "success" disabled>{user.btnText}</Button>
                            :
                            ""
                            }
                        </Card.Body>
